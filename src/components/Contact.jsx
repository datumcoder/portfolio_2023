...
const Contact = () => {
  ...
  <div class="w-full bg-white rounded shadow-lg p-8 m-4 md:max-w-sm md:mx-auto">
    ...
    <form id='contact' class="mb-4 md:flex md:flex-wrap md:justify-between" name="contact" method="POST" action="/success/">
      ...
      <div class="flex flex-col mb-4 md:w-full">
        ...
        <input required class="border py-2 px-3 text-grey-darkest md:mr-2" type="text" name="name" id="name"/>
      </div>
    
      <div class="flex flex-col mb-4 md:w-full">
        <label class="mb-2 uppercase font-bold text-lg text-grey-darkest" htmlFor="email">Email</label>
        <input required class="border py-2 px-3 text-grey-darkest" type="email" id="inputEmail" name="email" />
      </div>
      <div class="flex flex-col mb-6 md:w-full">
        <label className='uppercase text-sm py-2 font-bold'>Message</label>
            <textarea required className=' resize-none border-2 rounded-lg p-4' rows="7"  name='message'/>
      </div>
